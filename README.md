...
First, clone this repo with the name of the desired provider in place of `rand`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-rand
```

Second, replace references to `rand` with the name of your provider:
...
To use from Go, use `go get` to grab the latest version of the library
    ...
    $ go get github.com/roshankarande/pulumi-rand/sdk/go/...

## Configuration

The following configuration points are available for the `rand` provider:

- `rand:apiKey` (environment: `XYZ_API_KEY`) - the API key for `rand`
- `rand:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
